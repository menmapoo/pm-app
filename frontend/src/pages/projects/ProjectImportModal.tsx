...
const PROJECT_COLUMNS: ColumnDef[] = [
  ...
  { key: 'name', label: 'Name', render: v => <span className="font-medium text-slate-800">{v}</span> },
  {
    key: 'key', label: 'Key',
    render: v => (
      <code className="bg-blue-50 text-blue-700 px-1.5 py-0.5 rounded font-mono">{v}</code>
    ),
  },
  { key: 'description', label: 'Description', render: v => v || <span className="text-slate-300">—</span> },
  {
    key: 'status', label: 'Status',
    render: v => (
      <span className={`inline-flex px-1.5 py-0.5 rounded text-xs font-medium
        ${v === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'}`}>
...
export const ProjectImportModal: React.FC<Props> = ({ isOpen, onClose, onImported }) => {
  ...
  const handlePreview = async () => {
    ...
    setIsProcessing(false);
    try {
      const res = await projectImportApi.preview(selectedFile, (p) => {
        setUploadProgress(p);
  ...
  const handleImport = async () => {
    ...
    setImporting(true);
    try {
      const res = await projectImportApi.confirm(preview.validRows);
  ...
  const handleDownloadTemplate = async () => {
    try {
      const res = await projectImportApi.downloadTemplate();
      triggerBlobDownload(new Blob([res.data], { type: 'text/csv' }), 'projects_template.csv');
    } catch {
        ...
        <div className="grid grid-cols-2 gap-3 text-xs">
          <div>
            ...
            <ul className="space-y-1 text-slate-500">
              <li><code className="bg-white px-1 rounded border border-slate-200">name</code></li>
              <li><code className="bg-white px-1 rounded border border-slate-200">key</code> — unique uppercase key</li>
            </ul>
          </div>
          <div>
            ...
            <ul className="space-y-1 text-slate-500">
              ...
              <li><code className="bg-white px-1 rounded border border-slate-200">status</code> — active / archived</li>
            </ul>
          </div>
        </div>
    ...
    if (step === 'preview') {
      return (
        <>
          ...
          >
            Import {preview?.validRows.length ?? 0} Project{(preview?.validRows.length ?? 0) !== 1 ? 's' : ''}
